#ifndef GHOST_SCHEDULERS_EFS_EFS_SCHEDULER_H_
#define GHOST_SCHEDULERS_EFS_EFS_SCHEDULER_H_
...
// std::underlying_type<EfsTaskState>::type>(s) to go to an int, but even so,
...
class EfsTaskState {
 public:
  ...
  explicit EfsTaskState(State state)
      : EfsTaskState(state, OnRq::kMigrating, "") {}
  explicit EfsTaskState(State state, absl::string_view task_name)
      : EfsTaskState(state, OnRq::kMigrating, task_name) {}
  explicit EfsTaskState(State state, OnRq on_rq, absl::string_view task_name)
      : state_(state), on_rq_(on_rq), task_name_(task_name) {}

  // Make sure no one accidentally does something that'll mess up tracking like
  // task.task_state = EfsTaskState(EfsTaskState::kBlocked)
  EfsTaskState(const EfsTaskState&) = delete;
  EfsTaskState& operator=(const EfsTaskState&) = delete;
...
struct EfsTask : public Task<> {
  explicit EfsTask(Gtid d_task_gtid, ghost_sw_info sw_info)
  ...
  static bool Less(EfsTask* a, EfsTask* b) {
  ...
  EfsTaskState task_state =
      EfsTaskState(EfsTaskState::State::kBlocked,
                   EfsTaskState::OnRq::kDequeued,
                   gtid.describe());
  int cpu = -1;
  ...
  // Efs sorts tasks by vruntime, so we need to keep track of how long a task
  // has been running.
...
class EfsRq {
 public:
  explicit EfsRq();
  EfsRq(const EfsRq&) = delete;
  EfsRq& operator=(EfsRq&) = delete;

  // See EfsRq::granularity_ for a description of how these parameters work.
  ...
  void PutPrevTask(EfsTask* task) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);

  // DequeueTask 'task' from the runqueue. Task must be on rq.
  void DequeueTask(EfsTask* task) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);

  // The enqueued task with the smallest vruntime, or a nullptr if there are not
  // enqueued tasks.
  EfsTask* LeftmostRqTask() const ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    return rq_.empty() ? nullptr : *rq_.begin(); }

  // Attaches tasks to the run queue in batch.
  void AttachTasks(const std::vector<EfsTask*>& tasks_to_attach)
  ...
  int DetachTasks(const CpuState* dst_cs, int n,
                  std::vector<EfsTask*>& detached_tasks)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);
  ...
  // `can_migrate_task`.
  bool CanMigrateTask(EfsTask* task, const CpuState* dst_cs);
 ...
 private:
  ...
  void InsertTaskIntoRq(EfsTask* task) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);

  absl::Duration min_vruntime_ ABSL_GUARDED_BY(mu_);

  // Unlike in-kernel EFS, we want to have this properties per run-queue instead
  // of system wide.
  ...
  // data structure in EFS in the the kernel. While opaque, using an std::
  // container is easiest way to use a red-black tree short of writing or
  // importing our own.
  std::set<EfsTask*, decltype(&EfsTask::Less)> rq_ ABSL_GUARDED_BY(mu_);
...
class EfsMq {
 public:
  // Defines migration request of a EFS task.
  struct MigrationArg {
  ...
  EfsMq() = default;
  EfsMq(const EfsMq&) = delete;
  EfsMq& operator=(EfsMq&) = delete;

  void EnqueueTask(EfsTask* task) {
    mq_[task->gtid.id()] = { .task = task };
  }

  void EnqueueTask(EfsTask* task, int dst_cpu) {
    mq_[task->gtid.id()] = { .task = task, .dst_cpu = dst_cpu };
  }

  void DequeueTask(EfsTask* task) {
    mq_.erase(task->gtid.id());
  }

  void DequeueTaskIf(TryMigrateFn try_migrate) {
    absl::erase_if(mq_, [&try_migrate] (const auto& p) {
      const EfsMq::MigrationArg& arg = p.second;
...
struct CpuState {
  // current points to the EfsTask that we most recently picked to run on the
...
class EfsScheduler : public BasicDispatchScheduler<EfsTask> {
 public:
  ...
  int CountAllTasks() const {
    int num_tasks = 0;
    allocator()->ForEachTask([&num_tasks](Gtid gtid, const EfsTask* task) {
      ++num_tasks;
      return true;
    });
    return num_tasks;
 ...
 protected:
  ...
  // below.
  void TaskNew(EfsTask* task, const Message& msg) final;
  void TaskRunnable(EfsTask* task, const Message& msg) final;
  void TaskDeparted(EfsTask* task, const Message& msg) final;
  void TaskDead(EfsTask* task, const Message& msg) final;
  void TaskYield(EfsTask* task, const Message& msg) final;
  void TaskBlocked(EfsTask* task, const Message& msg) final;
  void TaskPreempted(EfsTask* task, const Message& msg) final;
  void TaskSwitchto(EfsTask* task, const Message& msg) final;
  void TaskAffinityChanged(EfsTask* task, const Message& msg) final;
  void TaskPriorityChanged(EfsTask* task, const Message& msg) final;
 ...
 private:
  ...
  // a side effect.
  void PutPrevTask(EfsTask* task);

  // EfsSchedule looks at the current cpu state and its run_queue, decides what
  // to run next, and then commits a txn. REQUIRES: Called after all messages
  // have been ack'ed otherwise the txn will fail.
  void EfsSchedule(const Cpu& cpu, BarrierToken agent_barrier, bool prio_boost);
  ...
  // Note: Should be called with this CPU's rq mutex lock held.
  void HandleTaskDone(EfsTask* task, bool from_switchto);
  ...
  void StartMigrateTask(EfsTask* task);
  ...
  inline EfsTask* NewIdleBalance(CpuState* cs);
  ...
  bool Migrate(EfsTask* task, Cpu cpu, BarrierToken seqnum);
  // Migrates pending tasks in the migration queue.
  void MigrateTasks(CpuState* cs);
  Cpu SelectTaskRq(EfsTask* task);
  void DumpAllTasks();
  ...
  CpuState* cpu_state_of(const EfsTask* task) {
    CHECK_GE(task->cpu, 0);
...
class EfsAgent : public LocalAgent {
 ...
 private:
  EfsScheduler* scheduler_;
};

class EfsConfig : public AgentConfig {
 public:
  EfsConfig() {}
  EfsConfig(Topology* topology, CpuList cpulist)
      : AgentConfig(topology, std::move(cpulist)) {}
  EfsConfig(Topology* topology, CpuList cpulist, absl::Duration min_granularity,
...
class FullEfsAgent : public FullAgent<EnclaveType> {
 public:
  ...
  ~FullEfsAgent() override { this->TerminateAgentTasks(); }

  std::unique_ptr<Agent> MakeAgent(const Cpu& cpu) override {
    return std::make_unique<EfsAgent>(&this->enclave_, cpu, scheduler_.get());
  }

  void RpcHandler(int64_t req, const AgentRpcArgs& args,
                  AgentRpcResponse& response) override {
    switch (req) {
      case EfsScheduler::kDebugRunqueue:
        scheduler_->debug_runqueue_ = true;
        response.response_code = 0;
        return;
      case EfsScheduler::kCountAllTasks:
 ...
 private:
  std::unique_ptr<EfsScheduler> scheduler_;
};

}  // namespace ghost

#endif  // GHOST_SCHEDULERS_EFS_EFS_SCHEDULER_H_
