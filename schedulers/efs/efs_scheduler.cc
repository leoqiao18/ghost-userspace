...
EfsScheduler::EfsScheduler(Enclave* enclave, CpuList cpulist,
  ...
  for (const Cpu& cpu : cpus()) {
    ...
    // This channel pointer is valid for the lifetime of EfsScheduler
    if (!default_channel_) {
...
// Our EFS agent has no notion of energy efficiency or scheduling domaims. So,
...
// In general, there are many, many, many heuristic in kernel EFS, so I tried to
...
Cpu EfsScheduler::SelectTaskRq(EfsTask* task) {
  ...
  if (eligible_cpus.Empty()) {
    DPRINT_EFS(3, absl::StrFormat("[%s]: No CPUs eligible for this task.",
                                  task->gtid.describe()));
...
void EfsScheduler::StartMigrateTask(EfsTask* task) {
  ...
  task->task_state.SetOnRq(EfsTaskState::OnRq::kMigrating);
...
void EfsScheduler::StartMigrateCurrTask() {
  ...
  cs->current = nullptr;
  task->task_state.SetState(EfsTaskState::State::kRunnable);
...
void EfsScheduler::TaskNew(EfsTask* task, const Message& msg) {
  ...
  CHECK_GE(payload->nice, EfsScheduler::kMinNice);
  CHECK_LE(payload->nice, EfsScheduler::kMaxNice);

  task->nice = payload->nice;
  task->weight =
      EfsScheduler::kNiceToWeight[task->nice - EfsScheduler::kMinNice];
  task->inverse_weight =
      EfsScheduler::kNiceToInverseWeight[task->nice - EfsScheduler::kMinNice];
...
void EfsScheduler::HandleTaskDone(EfsTask* task, bool from_switchto)
  ...
  EfsTaskState::State prev_state = task->task_state.GetState();
  task->task_state.SetState(EfsTaskState::State::kDone);

  if ((prev_state == EfsTaskState::State::kRunning || from_switchto) ||
      prev_state == EfsTaskState::State::kRunnable ||
      prev_state == EfsTaskState::State::kBlocked) {
    if (cs->current != task) {
...
void EfsScheduler::TaskBlocked(EfsTask* task, const Message& msg) {
  ...
  task->task_state.SetState(EfsTaskState::State::kBlocked);
...
void EfsScheduler::CpuTick(const Message& msg) {
  ...
  // CpuTick messages wake up the agent, EfsSchedule will eventually be
...
inline int EfsScheduler::FindBusiestQueue() {
...
void EfsScheduler::EfsSchedule(const Cpu& cpu, BarrierToken agent_barrier,
  ...
  if (prio_boost) {
    ...
    // priority than (kernel) EFS. The purpose of this is so that we can
  ...
  if (next) {
    DPRINT_EFS(3, absl::StrFormat("[%s]: Picked via PickNextTask",
                                  next->gtid.describe()));

    // std::cout << "min granularity: " << this->min_granularity_ << std::endl;
    // printf("EFS next: %d\n", next->gtid.tgid());
    // printf("EFS next cpu: %d\n", next->cpu);
    ...
    // by EFS dropping the rq->lock in PNT). During this window an agent
...
#ifndef NDEBUG
void EfsTaskState::AssertValidTransition(State next) {
  State curr = state_;
  ...
  if ((valid_states & (1 << static_cast<uint32_t>(curr))) == 0) {
    DPRINT_EFS(1, absl::StrFormat("[%s]: Cannot go from %s -> %s",
                                  absl::FormatStreamed(task_name_),
                                  absl::FormatStreamed(curr),
                                  absl::FormatStreamed(next)));
    DPRINT_EFS(1, absl::StrFormat("[%s]: Valid transitions -> %s are:",
                                  absl::FormatStreamed(task_name_),
                                  absl::FormatStreamed(next)));
...
void EfsTaskState::AssertValidTransition(OnRq next) {
  OnRq curr = on_rq_;
...
void EfsRq::SetMinGranularity(absl::Duration t) {
  min_preemption_granularity_ = t;
}

void EfsRq::SetLatency(absl::Duration t) { latency_ = t; }

absl::Duration EfsRq::MinPreemptionGranularity() {
...
void EfsRq::InsertTaskIntoRq(EfsTask* task) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
  ...
  DPRINT_EFS(2, absl::StrFormat("[%s]: Inserted into run queue",
                                task->gtid.describe()));
}
