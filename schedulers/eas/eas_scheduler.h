...
#ifndef GHOST_SCHEDULERS_EAS_EAS_SCHEDULER_H_
#define GHOST_SCHEDULERS_EAS_EAS_SCHEDULER_H_
...
// std::underlying_type<EasTaskState>::type>(s) to go to an int, but even so,
...
class EasTaskState {
 public:
  ...
  explicit EasTaskState(State state)
      : EasTaskState(state, OnRq::kMigrating, "") {}
  explicit EasTaskState(State state, absl::string_view task_name)
      : EasTaskState(state, OnRq::kMigrating, task_name) {}
  explicit EasTaskState(State state, OnRq on_rq, absl::string_view task_name)
      : state_(state), on_rq_(on_rq), task_name_(task_name) {}

  // Make sure no one accidentally does something that'll mess up tracking like
  // task.task_state = EasTaskState(EasTaskState::kBlocked)
  EasTaskState(const EasTaskState&) = delete;
  EasTaskState& operator=(const EasTaskState&) = delete;
...
struct EasTask : public Task<> {
  explicit EasTask(Gtid d_task_gtid, ghost_sw_info sw_info)
  ...
  EasTaskState task_state =
      EasTaskState(EasTaskState::State::kBlocked,
                   EasTaskState::OnRq::kDequeued,
                   gtid.describe());
  int cpu = -1;
  ...
  // Eas sorts tasks by vruntime, so we need to keep track of how long a task
  // has been running.
...
class EasMq {
 public:
  // Defines migration request of a EAS task.
  struct MigrationArg {
  ...
  EasMq() = default;
  EasMq(const EasMq&) = delete;
  EasMq& operator=(EasMq&) = delete;

  void EnqueueTask(EasTask* task) {
    mq_[task->gtid.id()] = { .task = task };
  }

  void EnqueueTask(EasTask* task, int dst_cpu) {
    mq_[task->gtid.id()] = { .task = task, .dst_cpu = dst_cpu };
  }

  void DequeueTask(EasTask* task) {
    mq_.erase(task->gtid.id());
  }

  void DequeueTaskIf(TryMigrateFn try_migrate) {
    absl::erase_if(mq_, [&try_migrate] (const auto& p) {
      const EasMq::MigrationArg& arg = p.second;
...
struct CpuState {
  // current points to the EasTask that we most recently picked to run on the
...
class EasScheduler : public BasicDispatchScheduler<EasTask> {
 ...
 protected:
  ...
  void TaskAffinityChanged(EasTask* task, const Message& msg) final;
  void TaskPriorityChanged(EasTask* task, const Message& msg) final;
 ...
 private:
  ...
  // a side effect.
  void PutPrevTask(EasTask* task);

  // EasSchedule looks at the current cpu state and its run_queue, decides what
  // to run next, and then commits a txn. REQUIRES: Called after all messages
  // have been ack'ed otherwise the txn will fail.
  void EasSchedule(const Cpu& cpu, BarrierToken agent_barrier, bool prio_boost);
  ...
  // Note: Should be called with this CPU's rq mutex lock held.
  void HandleTaskDone(EasTask* task, bool from_switchto);
  ...
  void StartMigrateTask(EasTask* task);
  ...
  inline EasTask* NewIdleBalance(CpuState* cs);
  ...
  bool Migrate(EasTask* task, Cpu cpu, BarrierToken seqnum);
  // Migrates pending tasks in the migration queue.
  void MigrateTasks(CpuState* cs);
  Cpu SelectTaskRq(EasTask* task);
  void DumpAllTasks();
  ...
  CpuState* cpu_state_of(const EasTask* task) {
    CHECK_GE(task->cpu, 0);
...
std::unique_ptr<EasScheduler> MultiThreadedEasScheduler(
...
class EasConfig : public AgentConfig {
 public:
  EasConfig() {}
  EasConfig(Topology* topology, CpuList cpulist)
      : AgentConfig(topology, std::move(cpulist)) {}
  EasConfig(Topology* topology, CpuList cpulist, absl::Duration min_granularity,
            absl::Duration latency)
      : AgentConfig(topology, std::move(cpulist)),
...
class FullEasAgent : public FullAgent<EnclaveType> {
 public:
  explicit FullEasAgent(EasConfig config) : FullAgent<EnclaveType>(config) {
    scheduler_ =
...
#endif  // GHOST_SCHEDULERS_EAS_EAS_SCHEDULER_H_
